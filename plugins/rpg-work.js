...
let handler = async (m, { conn, usedPrefix, command }) => {
  ...
  if (cooldowns[m.sender] && Date.now() - cooldowns[m.sender] < tiempoEspera * 1000) {
    ...
    m.reply(`❎ 𝐸𝑆𝑃𝐸𝑅𝐴 *${tiempoRestante}* 𝑃𝐴𝑅𝐴 𝑉𝑂𝐿𝑉𝐸𝑅 𝐴 𝑇𝑅𝐴𝐵𝐴𝐽𝐴𝑅.`)
    return
  }
   let work = works.getRandom()
   user.limit += amount
   await m.reply(`${work} *${amount} 🍡 𝐶𝐴𝑅𝐴𝑀𝐸𝐿𝑂𝑆.*`)
...
// Thanks to FG98
const works = [
   "LE Diste un sape a daniel.xyz por ser gay y ganaste",
   ...
   "Trabajas como secuestrador de negros y ganas",
